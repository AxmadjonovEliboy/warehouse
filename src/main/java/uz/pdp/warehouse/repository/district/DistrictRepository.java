...
//  @Query(value = "select * from organization where not is_deleted and id = :id", nativeQuery = true)
    ...
    @Query(value = " update district d set is_deleted = true , name = name||?2  where  d.id = ?1", nativeQuery = true)
    void deleteSoft(Long id, String key);

    @Query(value = "select * from district d where not is_deleted and d.id = ?1  ", nativeQuery = true)
    District getByIdAndNotDelete(Long id);

    @Query(value = "select * from district where not is_deleted", nativeQuery = true)
    List<District> findAllAndNotIsDelete();

    @Query(value = "select  * from district d where  not  is_deleted and d.id = ?1", nativeQuery = true)
    District getByAgentIdAndNotDelete(Long agentId);
}
